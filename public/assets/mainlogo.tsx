...
const MainLogo = ({ width = 148, height = 48 }) => {
  ...
  return (
    <svg width={width} height={height} viewBox="0 0 148 48" fill="none">
      <rect width="148" height="48" fill={`url(#pattern${id})`} />
      <defs>
        <pattern id={`pattern${id}`} patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlinkHref={`#image${id}`} transform="scale(0.00675676 0.0208333)" />
        </pattern>
        <image
          id={`image${id}`}
          width="148"
